    ...
    public class StockController : Controller
    {
        ...
        public async Task<IActionResult> StockEditPost([FromBody] EditStock value)
        {
            ...
            if (res.Contains("successfull"))
            {
                ...
                {
                    cdate = DateTime.Now,
                    message = "Stocke " + value.name + " Updated Sucessfully",
                    name = "Event",
                    uid = HttpContext.User.Identity.Name
                };
                res = await _log.InsertLogs(l);
        ...
        public async Task<IActionResult> MekeSalesWithBill([FromBody] SaleMake sale)
        {
            if (HttpContext.Session.GetString("sessionStock") != null)
            {
                ...
                i.isTaxed = true;
                var res =await _invoice.InsertInvoice(i);
                var p = new payments();
                p.amt = sale.paid;
                p.userid = "";
                p.status = "paid";
                ...
                p.sid = sl.sid;
                res =await _pyment.InsertPayments(p);

                res =await _sales.InsertSales(sl);
        ...
        public async Task<IActionResult> PayInvoice([FromBody]PayInvoice pay)
        {
            ...
            var res =await _pyment.InsertPayments(p);

            res =await _invoice.UpdateInvoice(inv);
            if (res.Contains("successfull"))
            {
                var result = new { status = res };
                var l = new log()
        ...
        public async Task<IActionResult> CancelPayment([FromBody] string id)
        {
            ...
            var inv = await _invoice.GetInvoiceByInvoiceID(py.invid);
            py.status = "Canceled";
            inv.status = "UnPaid";
            ...
            var res =await _invoice.UpdateInvoice(inv);
            res =await _pyment.UpdatePayments(py);
            if (res.Contains("successfull"))
            {
